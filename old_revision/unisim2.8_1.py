...
import string # Required for new 'mutate' sigil rule
...
class OctNode:
    ...
    def __init__(self, depth, page_index):
        ...
        self.fix_outcome_history = deque(maxlen=20) # History of anomaly fix outcomes for evolution - Increased history length
...
class SigilTransformer:
    ...
    def __init__(self):
        ...
        self.rules = {
            ...
            'mutate': lambda s: ''.join(random.choice(string.printable) if random.random() < 0.1 else c for c in s)  # New mutation rule
        }
        self.full_string_rules = ['invert', 'rotate', 'substitute', 'mutate'] # Add 'mutate'
    ...
    def transform(self, sigil, style='random', node=None, secure=False):
        ...
        """
        global cycle_num # Access global cycle_num
        ...
        # Force mutation if cycle_num > 10000 and diversity is low
        active_sigils = [n.st.sigil for n in roots if n and n.st and n.st.sigil]
        unique_sigils = len(set(active_sigils)) / max(1, len(active_sigils))
        if cycle_num > 10000 and unique_sigils < 0.5:
            chosen_style = 'mutate'
            Raw_Print(f"  [SIGIL] Forced mutation due to low diversity: {unique_sigils:.4f}")
        elif style == 'random':
        ...
        if chosen_style in self.full_string_rules:
            return self.rules[chosen_style](effective_sigil)[:SIGIL_LEN] # Truncate after transformation
...
class SharedSigilLedger:
    ...
    def record_mutation(self, old_sigil, new_sigil, page_idx, cycle):
        ...
        global voidEntropy
        # Penalize repeated sigils - Increase Entropy Costs for Repeated Sigils
        if self.sigil_mutation_history[old_sigil]['count'] > 5:
            entropy_cost *= 1.5
            Raw_Print(f"  [SIGIL] Increased entropy cost for repeated sigil: {old_sigil[:10]}...")
    ...
    def compute_semantic_vector(self, sigil):
        ...
        if total == 0:
            return [0.0] * 10
        vec = [b / total for b in bins]
        # Log for void anomalies
        if any(a.anomaly_type == 2 for a in newly_triggered_anomalies_queue):  # Void anomaly
            Raw_Print(f"  [SIGIL] Semantic vector for void anomaly sigil: {vec}")
        return vec
...
class ArchetypeEvolver:
    ...
    def evolve(self, node):
        ...
        if random.random() < node.social_cohesion * 0.25:  # Increased probability from 0.15 to 0.25
            # Calculate success ratio from recent anomaly fixes, extended history from 10 to 20
            success_ratio = sum(node.fix_outcome_history[-20:]) / len(node.fix_outcome_history[-20:]) if node.fix_outcome_history else 0.5
            if node.archetype_name in self.transitions:
                if success_ratio > 0.6:  # Lowered threshold from 0.7 to 0.6
                ...
                elif success_ratio < 0.4:  # Lowered threshold from 0.3 to 0.4
                    old_archetype = node.archetype_name
...
class CivilizationEvolver:
    ...
    def evolve(self, civ, node):
        ...
        if random.random() < 0.05:  # Increased probability from 0.01 to 0.05
            if civ.culture in self.evolutions:
                if civ.tech_level > 0.8 and node.stabilityPct > 0.6:  # Adjusted stability threshold from 0.7 to 0.6
                ...
                elif civ.tech_level < 0.3 and node.stabilityPct < 0.4:  # Adjusted stability threshold from 0.3 to 0.4
                    old_culture = civ.culture
...
def ElderGnosis_PredictRisk(predictor_obj, page_idx, anomaly_type, use_lstm=False):
    ...
    """
    global total_anomalies_triggered # Access global total_anomalies_triggered for dynamic activation

    # Dynamic Activation: Enable use_lstm=True in ElderGnosis_PredictRisk when total_anomalies_triggered > 15000.
    if total_anomalies_triggered > 15000:
        use_lstm = True
...
def HandleAnomaly(anomaly, force_action_type=None):
    ...
    # Emotional modifier for success probability
    emotion_modifier_for_success = {
        "resolute": 0.1, "curious": 0.05, "contemplative": 0.0, "guiding": 0.15,
        "observant": 0.05, "constructive": 0.1, "protective": 0.2, "confident": 0.15,
        "determined": 0.1, "intrigued": 0.05, "cautious": -0.05, "enlightened": 0.2,
        "pensive": -0.05, "inspiring": 0.15, "reflective": 0.0, "prescient": 0.25,
        "doubtful": -0.1, "innovative": 0.1, "frustrated": -0.15, "unyielding": 0.2,
        "weary": -0.2
    }.get(node.emotional_state, 0.0)
    ...
    if action == 'sigil':
        ...
        if random.random() < (0.7 - anomaly.severity * 0.3 + emotion_modifier_for_success) or force_action_type == 'stabilize':
        ...
        elif random.random() < (0.5 + emotion_modifier_for_success) or force_action_type == 'entangle':
        ...
        if random.random() < (0.6 - anomaly.severity * 0.2 + emotion_modifier_for_success) or force_action_type == 'stabilize':
            outcome_is_fixed = True
        ...
        if random.random() < (0.4 + emotion_modifier_for_success) or force_action_type == 'tunnel':
            outcome_is_fixed = True
        ...
        if random.random() < (0.5 - anomaly.severity * 0.25 + emotion_modifier_for_success) or force_action_type == 'resonate':
            outcome_is_fixed = True
...
def CrossPageInfluence(p_idx, target_idx):
    ...
    if p_idx != target_idx and target_idx < len(roots) and roots[target_idx] is not None:
        node = roots[p_idx]
        # Emotion-Driven Cross-Page Influence: Trigger more frequently for volatile emotions
        volatility_modifier = 1.5 if node.emotional_state in ["frustrated", "weary"] else 1.0
        influence = QuantumRand(cycle_num + p_idx + target_idx) * 0.1 * volatility_modifier
        ...
        Raw_Print(f"  Page {p_idx} influenced Page {target_idx} (Influence: {influence:.4f}, Emotion: {node.emotional_state})")
...
def sigil_resurrect(node, p_idx):
    ...
    if pageEigenstates.get(p_idx, 0) == 255: # Check if page is unmapped (collapsed)
        ...
        quantumHeapPages += 1
        node.stabilityPct = 0.5  # Reset stability
        Raw_Print(f"  [RESURRECT] Page {p_idx} restored via sigil!")
    node.st.sigil = sigil_transformer.transform(node.st.sigil, 'mutate', node=node) # Mutate sigil on resurrection
...
def VoidDecay(p_idx):
    ...
    if p_idx < len(roots) and roots[p_idx]:
        node = roots[p_idx]
        # Trigger Soft Collapses: Modify VoidDecay to unmap pages with low stability and high voidEntropy (>0.0081)
        if random.random() < voidEntropy * 0.2 and voidEntropy > 0.0081:  # Higher threshold
            ...
            if node.stabilityPct < 0.1 and pageEigenstates.get(p_idx, 0) == 1: # If very unstable and mapped
                ...
                if random.random() < 0.2:  # Chance to schedule resurrection
                    symbolic_echo_register.append({"cycle": cycle_num, "page_idx": p_idx, "event": "scheduled_resurrection"})
                    Raw_Print(f"  Page {p_idx} scheduled for sigil resurrection.")
    ...
    try:
        ...
        sigil_data = {
            sigil: {
                ...
                'count': info['count'],
                'mutations': info['mutations'], # Changed from 'mututations'
...
def calculate_archetype_collapse_ratio():
    ...
    collapse_ratio = total_failed_fixes / (total_successful_fixes + total_failed_fixes)
    # Add partial collapses from unmapped pages
    unmapped_pages = sum(1 for i in pageEigenstates if pageEigenstates[i] == 255)
    collapse_ratio += unmapped_pages * 0.01
    return min(1.0, collapse_ratio)
    ...
    try: # FIX: Wrap event processing in try-except
        ...
        for event in pygame.event.get():
            ...
            if cpu_cores_input and cpu_cores_input.handle_event(event): # FIX: Add check for cpu_cores_input existence
                try:
                    num_cpu_cores = max(1, min(int(cpu_cores_input.get_text()), 32)) # Cap CPU cores to os.cpu_count()
...
def _update_simulation_state(meta_omni_validator):
    ...
    # Prune Data Structures: Clear old entries every 10,000 cycles
    if cycle_num % 10000 == 0:
        symbolic_echo_register.clear()
        for page in anomaly_type_counts_per_page:
            anomaly_type_counts_per_page[page].clear()
        cross_page_influence_matrix.clear()
        Raw_Print(f"  [CLEANUP] Pruned data structures at cycle {cycle_num}")
...
def _log_and_visualize_data():
    ...
    # Reduce Pygame Overhead: Skip rendering every 10 cycles when simulation_speed_factor > 2.0
    if simulation_speed_factor > 2.0 and cycle_num % 10 != 0:
...
def CelestialOmniversePrimordialRite():
    ...
    # Optimize Multiprocessing: Cap num_cpu_cores to os.cpu_count()
    num_cpu_cores = min(num_cpu_cores, os.cpu_count() or 1)  # Cap CPU cores
...
class TestQuantumHeapTranscendence(unittest.TestCase):
    ...
    def test_sigil_transformation(self):
        ...
        # Test 'mutate' (new rule)
        # To test 'mutate' specifically, we need to ensure it applies mutations.
        # It's hard to assert the exact outcome because it's random, but we can check if it's different.
        original_sigil_for_mutate = "ABCDEFGHIJKLMNOP"
        transformed_mutated = transformer.transform(original_sigil_for_mutate, style='mutate')
        self.assertNotEqual(transformed_mutated, original_sigil_for_mutate, "Mutate transformation failed: sigil remained unchanged")
        self.assertEqual(len(transformed_mutated), SIGIL_LEN, "Mutated sigil length incorrect")
    ...
    def test_anomaly_handling(self):
        ...
        roots[0].stabilityPct = 0.5
        roots[0].fix_outcome_history = deque(maxlen=20) # Reset history for the test, use deque
